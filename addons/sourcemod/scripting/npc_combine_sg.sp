...
methodmap BaseNPC __nullable__
{
	...
	public int EquipItem(const char[] attachment, const char[] model, const char[] anim = "", int skin = 0)
	{
		...
		SetEntProp(item, Prop_Send, "m_nSkin", 1);
...
methodmap PetHeavy < BaseNPC
{
	public PetHeavy(int client, float vecPos[3], float vecAng[3], const char[] model, int team)
	{
		...
		brain.SetString("MoveAnim", "walkeasy_all", 64);
	...
	public void Attack()
	{
		...
		switch(this.m_iWeaponState)
		{
			...
			case AC_STATE_STARTFIRING:
			{
				...
				this.SetAnimation("runaimall1_sg");
				brain.SetString("MoveAnim", "runaimall1_sg", 64);
				brain.SetString("IdleAnim", "combatidle1", 64);
...
public Action Command_PetMenu(int client, int argc)
{
	...
	if(StrEqual(arg1, "squad", false))
	{
		...
		npc.Weapon = npc.EquipItem("lefthand", "models/weapons/w_shotgun.mdl", _, GetClientTeam(client) - 2);
	...
	else
	{
		...
		npc.Weapon = npc.EquipItem("lefthand", "models/weapons/w_shotgun.mdl", _, 8);
...
public void OnMapStart()
{
	...
	PrecacheSound(")weapons/shotgun/shotgun_fire6.wav");
...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_combine_shotgun", Command_PetMenu, ADMFLAG_ROOT);
	RegAdminCmd("sm_combine_shotgunno", Command_PetMenuNo, ADMFLAG_ROOT);
...
public Action OnBotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	if(damage >= GetEntProp(victim, Prop_Data, "m_iHealth"))
	{
		...
		strcopy(model, sizeof(model), gibs[0]);
			...
			SetEntProp(gib, Prop_Send, "m_nSkin", 1);
