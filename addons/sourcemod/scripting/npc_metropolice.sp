...
#define SOUND_COMBINE_STEP	(GetRandomInt(0, 5))? "npc/metropolice/gear1.wav":"npc/metropolice/gear2.wav":"npc/metropolice/gear3.wav":"npc/metropolice/gear4.wav":"npc/metropolice/gear5.wav":"npc/metropolice/gear6.wav"

#define SOUND_COMBINE_HURT	(GetRandomInt(0, 2))? "npc/metropolice/pain1.wav":"npc/metropolice/pain2.wav":"npc/metropolice/pain3.wav"
#define SOUND_COMBINE_DEATH	(GetRandomInt(0, 2))? "npc/metropolice/die1.wav":"npc/metropolice/die2.wav":"npc/metropolice/die3.wav"
...
methodmap PetHeavy < BaseNPC
{
	public PetHeavy(int client, float vecPos[3], float vecAng[3], const char[] model, int team)
	{
		...
		brain.SetString("MoveAnim", "walk_all", 64);
		brain.SetFloat("MoveSpeed", 85.0);
		brain.SetString("IdleAnim", "batonidle2", 64); 
		brain.SetFloat("OutOfRange", 400.0);
		//////////
		
		pet.CreatePather(client, 18.0, 18.0, 1000.0, MASK_NPCSOLID | MASK_PLAYERSOLID, 60.0, 0.65, 0.65);
		pet.SetAnimation("walk_all");
	...
	public void Attack()
	{
		...
		switch(this.m_iWeaponState)
		{
			case AC_STATE_IDLE:
			{
				...
				brain.SetFloat("MoveSpeed", 85.0);
				this.SetAnimation("walk_all");
				SetEntPropFloat(this.index, Prop_Data, "m_speed", 85.0);
				brain.SetString("IdleAnim", "pistolangryidle2", 64);
			...
			case AC_STATE_STARTFIRING:
			{
				...
				this.SetAnimation("drawpistol");
				
				brain.SetFloat("MoveSpeed", 125.0);
				SetEntPropFloat(this.index, Prop_Data, "m_speed", 125.0);
			...
			case AC_STATE_FIRING:
			{
				...
				this.SetAnimation("run_aiming_pistol_all");
				brain.SetString("MoveAnim", "run_aiming_pistol_all", 64);
				brain.SetString("IdleAnim", "pistolangryidle2", 64);
				brain.SetFloat("MoveSpeed", 125.0);
				SetEntPropFloat(this.index, Prop_Data, "m_speed", 125.0);
				if (this.m_iWeaponMode == TF_WEAPON_SECONDARY_MODE)
				...
				else
				{
					...
					if (GetGameTime() >= this.m_flNextPrimaryAttack)
					{
						...
						for (int i = 0; i < 4; i++)
						{
							...
							EmitSoundToAll("^weapons/pistol/pistol_fire3.wav", this.index, SNDCHAN_WEAPON, 95, _, SNDVOL_NORMAL, GetRandomInt(97, 103)) 
							FireBullet(this.index, this.Weapon, WorldSpaceCenter(this.index), vecDir, 2.5, 9000.0, DMG_BULLET, "bullet_tracer02_blue");
						}
						...
						this.m_flNextPrimaryAttack = GetGameTime() + GetRandomFloat(0.08,1.5);
					}
				}
			}
...
public void PetHeavyThink(int iEntity)
{
	...
	if(KnownEntity != Address_Null)
	{
		...
		else
		{
			...
			npc.SetAnimation("walk_all");
			SetEntPropFloat(npc.index, Prop_Data, "m_speed", 125.0);
		}
	}
...
public void Blend9Think(int iEntity)
{
	...
	int m_iMoveY = npc.LookupPoseParameter("move_yaw");
...
public void OnEntityDestroyed(int entity)
{
	...
	{
		StopSound(entity, SNDCHAN_STATIC, ")weapons/minigun_shoot.wav");
		StopSound(entity, SNDCHAN_STATIC, ")weapons/minigun_spin.wav");
...
public Action Command_PetMenu(int client, int argc)
{
	...
	if(StrEqual(arg1, "squad", false))
	{
		...
		PetHeavy npc = new PetHeavy(client, flPos, flAng, "models/combine_soldier.mdl", GetClientTeam(client));	
		npc.Weapon = npc.EquipItem("lefthand", "models/weapons/w_pistol.mdl", _, GetClientTeam(client) - 2);
		Dynamic squad = npc.m_Squad = CTFBotSquad(npc.index);
		
		BaseNPC	iLeader = npc;
		
		npc = new PetHeavy(client, flPos, flAng, "models/combine_soldier.mdl", GetClientTeam(client));	
		npc.Weapon = npc.EquipItem("lefthand", "models/weapons/w_pistol.mdl", _, GetClientTeam(client) - 2);
		SetVariantString("1.0"); AcceptEntityInput(npc.index, "SetModelScale");
	...
	else
	{
		...
		PetHeavy npc = new PetHeavy(client, flPos, flAng, "models/combine_soldier.mdl", iTeam);	
		npc.Weapon = npc.EquipItem("lefthand", "models/weapons/w_pistol.mdl", _, 8);
...
public void OnMapStart()
{
	...
	PrecacheSound("^weapons/pistol/pistol_fire3.wav");
	PrecacheSound("npc/metropolice/gear1.wav");
	PrecacheSound("npc/metropolice/die1.wav");
	PrecacheSound("npc/metropolice/die2.wav");
	PrecacheSound("npc/metropolice/die3.wav");
	PrecacheSound("npc/metropolice/pain1.wav");
	PrecacheSound("npc/metropolice/pain2.wav");
	PrecacheSound("npc/metropolice/pain3.wav");
	PrecacheSound("npc/metropolice/gear2.wav");
	PrecacheSound("npc/metropolice/gear3.wav");
	PrecacheSound("npc/metropolice/gear4.wav");
	PrecacheSound("npc/metropolice/gear5.wav");
	PrecacheSound("npc/metropolice/gear6.wav");
...
public void OnPluginStart()
{
	...
	g_hDebug                 = CreateConVar("sm_metropolicenpc_debug",                      "0.0",    "Debug aiming", _, true, 0.0, true, 1.0);
	g_hAimRate               = CreateConVar("sm_metropolicenpc_head_aim_tracking_interval", "0.25",   "Aim Recalculate Interval");
	g_hHeadSteadyRate        = CreateConVar("sm_metropolicenpc_head_aim_steady_max_rate",   "100.0",  "Head aim steady max rate");
	g_hSaccadeSpeed          = CreateConVar("sm_metropolicenpc_saccade_speed",              "1000.0", "Max head angular velocity");
	g_hHeadResettleAngle     = CreateConVar("sm_metropolicenpc_head_resettle_angle",        "100.0",  "After rotating through this angle, the bot pauses to 'recenter' its virtual mouse on its virtual mousepad");
	g_hHeadResettleTime      = CreateConVar("sm_metropolicenpc_head_resettle_time",         "0.3",    "How long the bot pauses to 'recenter' its virtual mouse on its virtual mousepad");
	g_hHeadAimSettleDuration = CreateConVar("sm_metropolicenpc_head_aim_settle_duration",   "0.3",    "");

	RegAdminCmd("sm_metropolice", Command_PetMenu, ADMFLAG_ROOT);
	RegAdminCmd("sm_metropoliceno", Command_PetMenuNo, ADMFLAG_ROOT);
...
public MRESReturn CBaseAnimating_HandleAnimEvent(int pThis, Handle hParams)
{
	...
	Format(sound, sizeof(sound), "npc/metropolice/gear%i.wav", GetRandomInt(1,6));
	if(event == 6004)	//Footstep
	{
		EmitSoundToAll(sound, pThis);
	}
	if(event == 6005)	//Footstep
	{
		EmitSoundToAll(sound, pThis);
	}
...
public Action OnBotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	char sound[PLATFORM_MAX_PATH];
	char sound_[PLATFORM_MAX_PATH];
	Format(sound, sizeof(sound), "npc/metropolice/pain%i.wav", GetRandomInt(1,3));
	Format(sound_, sizeof(sound_), "npc/metropolice/die%i.wav", GetRandomInt(1,3));
	if(view_as<BaseNPC>(attacker).GetTeam() != view_as<BaseNPC>(victim).GetTeam())
	{
		...
		EmitSoundToAll(sound, victim, SNDCHAN_VOICE)
	}
	
	if(damage >= GetEntProp(victim, Prop_Data, "m_iHealth"))
	{
		EmitSoundToAll(sound_, victim, SNDCHAN_VOICE)
		...
		strcopy(model, sizeof(model), gibs[0]);
			...
			int gib = CreateEntityByName("prop_ragdoll");
			DispatchKeyValue(gib, "model", "models/police.mdl");
