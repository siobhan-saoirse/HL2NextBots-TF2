...
Handle g_hHandleAnimEvent2;

char g_FootstepSounds[][] = {
...
char g_MeleeHitSounds[][] = {
	"player/tank/hit/hulk_punch_1.wav",
};
...
{
	RegAdminCmd("sm_tank", Command_PetMenu, ADMFLAG_ROOT);
	
	Handle hConf = LoadGameConfigFile("tf2.pets");
	//DHooks
	g_hHandleAnimEvent2    = DHookCreateEx2(hConf, "CBaseAnimating::HandleAnimEvent",  HookType_Entity, ReturnType_Void,   ThisPointer_CBaseEntity, CBaseAnimating_HandleAnimEvent2);
	DHookAddParam(g_hHandleAnimEvent2, HookParamType_ObjectPtr, -1);
	InitGamedata();

	delete hConf;
}

Handle DHookCreateEx2(Handle gc, const char[] key, HookType hooktype, ReturnType returntype, ThisPointerType thistype, DHookCallback callback)
{
...
public void OnMapStart()
{
	for (int i = 0; i < (sizeof(g_DeathSounds));       i++) { 
		...
		char sound[512];
		Format(sound, sizeof(sound), "sound/%s", g_DeathSounds[i]);
		AddFileToDownloadsTable(sound);
	}
	for (int i = 0; i < (sizeof(g_FootstepSounds));       i++) { 
		PrecacheSound(g_FootstepSounds[i]);       
		char sound[512];
		Format(sound, sizeof(sound), "sound/%s", g_FootstepSounds[i]);
		AddFileToDownloadsTable(sound);
	}
	for (int i = 0; i < (sizeof(g_HurtSounds));        i++) { 
		PrecacheSound(g_HurtSounds[i]);        
		char sound[512];
		Format(sound, sizeof(sound), "sound/%s", g_HurtSounds[i]);
		AddFileToDownloadsTable(sound);
	}
	for (int i = 0; i < (sizeof(g_IdleSounds));        i++) { 
		PrecacheSound(g_IdleSounds[i]);        
		char sound[512];
		Format(sound, sizeof(sound), "sound/%s", g_IdleSounds[i]);
		AddFileToDownloadsTable(sound);
	}
	for (int i = 0; i < (sizeof(g_IdleAlertedSounds)); i++) { 
		PrecacheSound(g_IdleAlertedSounds[i]); 
		char sound[512];
		Format(sound, sizeof(sound), "sound/%s", g_IdleAlertedSounds[i]);
		AddFileToDownloadsTable(sound);
	}
	for (int i = 0; i < (sizeof(g_MeleeHitSounds));    i++) { 
		PrecacheSound(g_MeleeHitSounds[i]);  
		char sound[512];
		Format(sound, sizeof(sound), "sound/%s", g_MeleeHitSounds[i]);
		AddFileToDownloadsTable(sound);  
	}
	for (int i = 0; i < (sizeof(g_MeleeAttackSounds));    i++) { 
		PrecacheSound(g_MeleeAttackSounds[i]);    
		char sound[512];
		Format(sound, sizeof(sound), "sound/%s", g_MeleeAttackSounds[i]);
		AddFileToDownloadsTable(sound);  
	}
	for (int i = 0; i < (sizeof(g_MeleeMissSounds));   i++) { PrecacheSound(g_MeleeMissSounds[i]);   }

	AddFileToDownloadsTable("materials/models/infected/common/l4d2/ci_burning.vtf");
	...
	AddFileToDownloadsTable("materials/models/survivors/coach/coach_head_wrp.vtf");
	AddFileToDownloadsTable("models/infected/anim_hulk.ani");
	AddFileToDownloadsTable("models/infected/anim_hulk.mdl");
	AddFileToDownloadsTable("models/infected/hulk.dx80.vtx");
	AddFileToDownloadsTable("models/infected/hulk.dx90.vtx");
	AddFileToDownloadsTable("models/infected/hulk.mdl");
	AddFileToDownloadsTable("models/infected/hulk.phy");
	AddFileToDownloadsTable("models/infected/hulk.sw.vtx");
	AddFileToDownloadsTable("models/infected/hulk.vvd");
...
methodmap Clot < CClotBody
{
	...
	public void PlayMeleeHitSound() {
		EmitSoundToAll(g_MeleeHitSounds[GetRandomInt(0, sizeof(g_MeleeHitSounds) - 1)], this.index, SNDCHAN_STATIC, 95, _, 1.0, GetRandomInt(95, 105));
	...
	public float GetRunSpeed()      { return this.IsAlert() ? 210.0 : 210.0 * 0.45; }
	public float GetMaxJumpHeight() { return 90.0; }
	public float GetLeadRadius()    { return 500.0; }
	
	public Clot(int client, float vecPos[3], float vecAng[3], const char[] model)
	{
		Clot npc = view_as<Clot>(CBaseActorTank(vecPos, vecAng, model, "1.0", "16000"));
		
		int iActivity = npc.LookupActivity("ACT_MP_STAND_MELEE");
		if(iActivity > 0) npc.StartActivity(iActivity);
		
		//Animevents 
		DHookEntity(g_hHandleAnimEvent2,  true, npc);
		...
		npc.m_flNextPainSound = npc.m_flNextTargetTime;
	...
	public bool DoSwingTrace(Handle &trace)
	{
		...
		float vecSwingEnd[3];
		vecSwingEnd[0] = vecSwingStart[0] + vecForward[0] * 150;
		vecSwingEnd[1] = vecSwingStart[1] + vecForward[1] * 150;
...
public MRESReturn CBaseAnimating_HandleAnimEvent2(int pThis, Handle hParams)
{
	int event = DHookGetParamObjectPtrVar(hParams, 1, 0, ObjectValueType_Int);

	char sound[PLATFORM_MAX_PATH];
	Format(sound, sizeof(sound), "player/footsteps/tank/walk/tank_walk0%i.wav", GetRandomInt(1,6));
...
public void ClotThink(int iNPC)
{
	...
	if(PrimaryThreat.Address != Address_Null)
	{
		...
		else
		{
			...
			if(flDistanceToTarget < 120 && !npc.IsPlayingGesture("ACT_FLINCH"))
			{
					...
					npc.AddGesture("ACT_MP_ATTACK_STAND_MELEE");
						...
						if(npc.DoSwingTrace(swingTrace)) 
						{
							...
							if(target > 0) 
							{
								...
								{
									TF2_StunPlayer(target, 1.5, 0.8, TF_STUNFLAG_SLOWDOWN);
								}
						...
						delete swingTrace;
					npc.m_flNextMeleeAttack = GetGameTime() + 2;
				}
				if (flDistanceToTarget < 80) {
	...
	if(!npc.m_bJumping)
	{
		if(npc.m_bPathing) {
			if(npc.IsAlert() && !npc.IsDecapitated()) {
				...
				idealActivity = npc.LookupActivity("ACT_MP_WALK_MELEE");
			}
		} else {
...
public Action ClotDamaged(int victim, int& attacker, int& inflictor, float& damage, int& damagetype, int& ammotype, int hitbox, int hitgroup)
{
	...
	//Friendly fire
	if(view_as<CBaseActorTank>(attacker).GetTeam() == view_as<CBaseActorTank>(victim).GetTeam())
	...
	if(hitgroup == HITGROUP_HEAD)
	{
		...
		if(!npc.IsDecapitated())
		{
			if(!npc.IsPlayingGesture("ACT_MP_GESTURE_FLINCH_CHEST"))
			{
				...
				if(npc.m_flNextPainSound < GetGameTime())
				{
					npc.PlayHurtSound();
					npc.m_flNextPainSound = GetGameTime() + 0.7;
				}
			}
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && IsPlayerAlive(i) && GetRandomInt(1,8) == 1)
		{
			int client = i;
			SetVariantString("randomnum:100");
			AcceptEntityInput(client, "AddContext");
...
public Action Command_PetMenu(int client, int argc)
{
	...
	Clot(client, flPos, flAng, "models/infected/hulk.mdl");
