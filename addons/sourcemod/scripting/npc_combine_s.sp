...
#define SOUND_COMBINE_STEP	(GetRandomInt(0, 5))? "npc/combine_soldier/gear1.wav":"npc/combine_soldier/gear2.wav":"npc/combine_soldier/gear3.wav":"npc/combine_soldier/gear4.wav":"npc/combine_soldier/gear5.wav":"npc/combine_soldier/gear6.wav"

#define SOUND_COMBINE_HURT	(GetRandomInt(0, 2))? "npc/combine_soldier/pain1.wav":"npc/combine_soldier/pain2.wav":"npc/combine_soldier/pain3.wav"
#define SOUND_COMBINE_DEATH	(GetRandomInt(0, 2))? "npc/combine_soldier/die1.wav":"npc/combine_soldier/die2.wav":"npc/combine_soldier/die3.wav"
...
methodmap PetHeavy < BaseNPC
{
	public PetHeavy(int client, float vecPos[3], float vecAng[3], const char[] model, int team)
	{
		...
		brain.SetString("IdleAnim", "combatidle1", 64); 
		brain.SetFloat("OutOfRange", 400.0);
		//////////
	...
	public void Attack()
	{
		...
		switch(this.m_iWeaponState)
		{
			...
			case AC_STATE_STARTFIRING:
			{
				...
				this.SetAnimation("runaimall1");
				brain.SetString("MoveAnim", "runaimall1", 64);
				brain.SetString("IdleAnim", "combatidle1", 64);
				...
				else
				{
					...
					if (GetGameTime() >= this.m_flNextPrimaryAttack)
					{
						...
						for (int i = 0; i < 4; i++)
						{
							...
							EmitSoundToAll("^weapons/ar1/ar1_dist1.wav", this.index, SNDCHAN_WEAPON, 95, _, SNDVOL_NORMAL, GetRandomInt(97, 103)) 
						...
						view_as<BaseNPC>(this.Weapon).PlayGesture("fire");
						this.PlayGesture("gesture_shoot_ar1");
						this.m_flTimeWeaponIdle = GetGameTime() + 1;
...
public Action Command_PetMenu(int client, int argc)
{
	...
	if(StrEqual(arg1, "squad", false))
	{
		...
		PetHeavy npc = new PetHeavy(client, flPos, flAng, "models/combine_super_soldier.mdl", GetClientTeam(client));	
		npc.Weapon = npc.EquipItem("lefthand", "models/weapons/w_irifle.mdl", _, GetClientTeam(client) - 2);
		Dynamic squad = npc.m_Squad = CTFBotSquad(npc.index);
		
		BaseNPC	iLeader = npc;
		
		npc = new PetHeavy(client, flPos, flAng, "models/combine_super_soldier.mdl", GetClientTeam(client));	
		npc.Weapon = npc.EquipItem("lefthand", "models/weapons/w_irifle.mdl", _, GetClientTeam(client) - 2);
	...
	else
	{
		...
		PetHeavy npc = new PetHeavy(client, flPos, flAng, "models/combine_super_soldier.mdl", iTeam);	
		npc.Weapon = npc.EquipItem("lefthand", "models/weapons/w_irifle.mdl", _, 8);
...
public void OnPluginStart()
{
	...
	g_hDebug                 = CreateConVar("sm_combine_snpc_debug",                      "0.0",    "Debug aiming", _, true, 0.0, true, 1.0);
	g_hAimRate               = CreateConVar("sm_combine_snpc_head_aim_tracking_interval", "0.25",   "Aim Recalculate Interval");
	g_hHeadSteadyRate        = CreateConVar("sm_combine_snpc_head_aim_steady_max_rate",   "100.0",  "Head aim steady max rate");
	g_hSaccadeSpeed          = CreateConVar("sm_combine_snpc_saccade_speed",              "1000.0", "Max head angular velocity");
	g_hHeadResettleAngle     = CreateConVar("sm_combine_snpc_head_resettle_angle",        "100.0",  "After rotating through this angle, the bot pauses to 'recenter' its virtual mouse on its virtual mousepad");
	g_hHeadResettleTime      = CreateConVar("sm_combine_snpc_head_resettle_time",         "0.3",    "How long the bot pauses to 'recenter' its virtual mouse on its virtual mousepad");
	g_hHeadAimSettleDuration = CreateConVar("sm_combine_snpc_head_aim_settle_duration",   "0.3",    "");

	RegAdminCmd("sm_combine_super", Command_PetMenu, ADMFLAG_ROOT);
	RegAdminCmd("sm_combine_superno", Command_PetMenuNo, ADMFLAG_ROOT);
...
public MRESReturn CBaseAnimating_HandleAnimEvent(int pThis, Handle hParams)
{
	...
	Format(sound, sizeof(sound), "npc/combine_soldier/gear%i.wav", GetRandomInt(1,6));
...
public Action OnBotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	Format(sound, sizeof(sound), "npc/combine_soldier/pain%i.wav", GetRandomInt(1,3));
	Format(sound_, sizeof(sound_), "npc/combine_soldier/die%i.wav", GetRandomInt(1,3));
	...
	if(damage >= GetEntProp(victim, Prop_Data, "m_iHealth"))
	{
		...
		strcopy(model, sizeof(model), gibs[0]);
			...
			DispatchKeyValue(gib, "model", "models/combine_super_soldier.mdl");
